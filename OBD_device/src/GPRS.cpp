...
GPRS::GPRS() {
	...
	Chip_SWM_MovablePortPinAssign(SWM_UART2_RXD_I, 1, 10);
	Chip_SWM_MovablePortPinAssign(SWM_UART2_TXD_O, 1, 9);
	...
	Chip_UART_SetBaud(LPC_USART, 19200);
...
int GPRS::write(const char* buf, int len) {
	Chip_UART_SendRB(LPC_USART, &txring, buf, len);
	return len;
}
int GPRS::print(int val, int format) {
	uint8_t byte = val;
