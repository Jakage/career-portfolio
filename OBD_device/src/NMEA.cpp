...
NMEA::NMEA() {
	...
	Chip_SWM_MovablePortPinAssign(SWM_UART1_RXD_I, 0, 29);
	Chip_SWM_MovablePortPinAssign(SWM_UART1_TXD_O, 0, 9);
	...
	Chip_UART_SetBaud(LPC_USART, 4800); // 4800 for Fastrax, 9600 for Maestro
...
// Auto generated write/print functions, not in use but can be used for changing the bandwidth
int NMEA::write(const char* buf, int len) {
	Chip_UART_SendRB(LPC_USART, &txring, buf, len);
	return len;
}
int NMEA::print(int val, int format) {
	uint8_t byte = val;
