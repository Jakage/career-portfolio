...
MPU9250::MPU9250(int deviceNumber, int speed) {
	if(deviceNumber == 0) {
		device = LPC_I2C0;
		...
		// currently LPC15xx supports only I2C number 0
	}
...
bool MPU9250::transaction(uint8_t devAddr, uint8_t *txBuffPtr, uint16_t txSize, uint8_t *rxBuffPtr, uint16_t rxSize) {
...
void MPU9250::readAccelData(int16_t *destination) {
	...
	if(transaction(MPU9250_ADDRESS, &ACCEL_XOUT_H, 1, rawData, 6)) {
...
void MPU9250::writeByte(uint8_t devAddr, uint8_t regAddr, uint8_t data) {
	...
	dataArray[0] = regAddr;
	dataArray[1] = data;
	transaction(devAddr, dataArray, 2, 0, 0);
}

uint8_t MPU9250::readByte(uint8_t deviceAddress, uint8_t registerAddress) {
	uint8_t data;
	transaction(deviceAddress, &registerAddress, 1, &data, 1);
	return data;
}

void MPU9250::readBytes(uint8_t deviceAddress, uint8_t registerAddress, uint8_t count, uint8_t *dest) {
	transaction(deviceAddress, &registerAddress, 1, dest, count);
}
